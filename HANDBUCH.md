...
Der Hinweis `Neu in Version 0.3.12` erscheint nach dem Update pro Browser beziehungsweise Geraet automatisch nur einmal. `Verstanden` merkt die bestaetigte Version lokal; auch nach einem Neuladen bleibt der Hinweis geschlossen. `Nach Update suchen` oeffnet die Hinweise zur aktuell geladenen Version jederzeit bewusst erneut.
...
Fuer den Nachfolgekandidaten `0.3.12` gilt die Versionsbindung an die bestehende Liveversion `0.3.11` und die Kandidatenversion `0.3.12`. Die separat dokumentierte Backup-Schemakorrektur ist Bestandteil dieses Kandidaten; die Versionsvorbereitung fuegt ihr keine weitere Funktion hinzu und erteilt keine Deployfreigabe.
