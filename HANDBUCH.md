...
Der Hinweis `Neu in Version 0.3.11` erscheint nach dem Update pro Browser beziehungsweise Geraet automatisch nur einmal. `Verstanden` merkt die bestaetigte Version lokal; auch nach einem Neuladen bleibt der Hinweis geschlossen. `Nach Update suchen` oeffnet die Hinweise zur aktuell geladenen Version jederzeit bewusst erneut.
...
Fuer den Nachfolgekandidaten `0.3.11` bleibt dieser Backup-Ablauf unveraendert; ausschliesslich die Versionsbindung wird auf die bestehende Liveversion `0.3.10` und die Kandidatenversion `0.3.11` fortgeschrieben.
