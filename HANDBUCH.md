...
Die App ist als PWA installierbar. Auf unterstuetzten Geraeten kann sie aus dem Browser zum Home-Bildschirm hinzugefuegt werden und startet danach wie eine normale App. Die Kopfzeile ordnet Navigation, Hauswahl und Kontoaktionen auf schmalen Desktop-App-Fenstern untereinander an, statt Inhalte rechts abzuschneiden; die Seite benoetigt dafuer keinen horizontalen Scrollbereich.
...
| Tagesdetails | Beim Ueberfahren, Tastaturfokus oder Antippen alle drei Slots mit freien, belegten und eigenen Geraeten anzeigen; belegte Normal- und Dauertermine zeigen den verwalteten Wohnungs-/Klingelschildnamen im berechtigten Haus, eigene Termine bleiben als eigene Buchung erkennbar |
...
Der persoenliche Kalenderfeed unter `Einstellungen -> App & Geraet` ist nur fuer aktive Bewohnerkonten mit aktiver Wohnung verfuegbar; dies schliesst kombinierte Bewohner-/Hausadmin-Konten ein, reine Admin-Konten aber aus. Er enthaelt eigene beziehungsweise gemeinsam ueber die Wohnung gebuchte Normalbuchungen und der Wohnung zugeordnete Dauertermine. Die Bedienung zeigt drei Schritte: geheime Abo-Adresse erstellen, kopieren oder direkt in der Kalender-App oeffnen und dort `Kalender abonnieren` beziehungsweise `Per URL hinzufuegen` waehlen. Die direkte Aktion verwendet das uebliche `webcal:`-Schema; die kopierte HTTPS-Adresse bleibt der universelle Rueckfall. Die geheime Adresse wird ausschliesslich unmittelbar nach Erstellen oder Ersetzen angezeigt und beim Schliessen, erneuten Oeffnen, Statusneuladen, Widerrufen und Abmelden vollstaendig aus Eingabe und Link entfernt; insbesondere bleibt kein altes Linkziel im DOM. Serverseitig liegt nur ihr SHA-256-Hash. Ersetzen widerruft die alte Adresse, Widerrufen sperrt sie sofort. Unicode-Bezeichnungen werden RFC-5545-konform und ohne Zeichenbeschaedigung gefaltet; fremde Namen, Kontakte und Buchungen fehlen.
...
Der Hinweis `Neu in Version 0.3.13` erscheint nach dem Update pro Browser beziehungsweise Geraet automatisch nur einmal. `Verstanden` merkt die bestaetigte Version lokal; auch nach einem Neuladen bleibt der Hinweis geschlossen. `Nach Update suchen` oeffnet die Hinweise zur aktuell geladenen Version jederzeit bewusst erneut.
...
| Erfolgreiche Pruefung dokumentieren und damit atomar freigeben/abschliessen | Nein | Ja | Ja |
...
Nach aussen gibt es ausschliesslich die drei Hauptstatus `Neu`, `In Bearbeitung` und `Erledigt`. Beim Uebernehmen eines neuen Falls waehlt der Admin ohne Vorauswahl ausdruecklich `Ressource sperren` oder `Ressource nicht sperren`; Auswahl, Statuswechsel und eine optionale Sperre werden gemeinsam gespeichert. Der erste Wechsel aus `Neu` ist ausschliesslich ueber diese atomare Uebernahme erlaubt. Eine direkte Tagebuchaktion `block` oder eine allgemeine Ressourcenbearbeitung darf den Entscheid weder ersetzen noch den Fall implizit uebernehmen; bei einem offenen neuen Fall wird der Umgehungsversuch ohne Teilmutation abgewiesen. Eine Sperre ist damit kein eigener Hauptstatus. Danach folgt der kompakte Betriebsnachweis `Reparatur -> Funktionspruefung`. Die Oberflaeche bietet die Funktionspruefung erst an, nachdem mindestens eine Reparatur dokumentiert wurde; so kann auch ein ohne Sperre uebernommener Fall nicht versehentlich einen ungueltigen Schritt absenden. Eine nicht erfolgreiche Funktionspruefung laesst Fall und fallbezogene Sperre unveraendert in Bearbeitung. Eine erfolgreiche Funktionspruefung ist zugleich der nachvollziehbare Abschlussnachweis: Der Server speichert Pruefergebnis und Dokumentation, schliesst den Fall und gibt eine fallbezogen gesperrte Ressource atomar frei. War sie nie gesperrt, bleibt sie aktiv und nur der gepruefte Fall wird abgeschlossen. Eine dritte Abschlussnotiz ist nicht erforderlich. Historische Faelle, die bereits im alten Zwischenstatus `tested` stehen, koennen weiterhin ueber die bisherigen Aktionen freigegeben oder abgeschlossen werden. Abgeschlossene Faelle koennen nicht mehr ergaenzt, veraendert oder geloescht werden. Weitere Bewohnermeldungen zur gleichen Ressource werden dem bereits offenen neutralen Fall zugeordnet, bleiben aber als getrennte persoenliche Berichte erhalten. Der Haus-Admin arbeitet nur im eigenen Haus. Der Superadmin waehlt ein Haus bewusst aus und sieht oder bearbeitet danach ausschliesslich dessen Faelle; ein globales Superadminrecht allein erweitert weder die aktive Hausabfrage noch die Admin-Push-Empfaengergruppe.
...
Nach erfolgreicher Uebernahme eines neuen Stoerungsfalls wechselt ein zuvor aktiver Filter `Neu` automatisch zu `In Bearbeitung`, damit der uebernommene Fall sichtbar bleibt. Eine fehlgeschlagene Uebernahme veraendert den Filter nicht. Andere bewusst gewaehlte Filter bleiben erhalten.
...
Der kontrollierte Produktionsbootstrap verwendet `node scripts/bootstrap-production-backup.js` nur in einem getrennt freigegebenen Wartungsfenster. Vor jeder Dateisystemwirkung muessen die von Render selbst gesetzte Service-ID und Commitrevision zu `washraum-app`, erwarteter voller Livecommit, bestehende Liveversion `0.3.11`, Kandidatenversion `0.3.13`, Quelldatei `/var/data/washraum.sqlite`, Kandidatencommit und der daraus eindeutig gebildete neue Zielname unter `/var/data/backups` exakt stimmen. Das aus genau drei Skripten und `production-backup-bundle.sha256` bestehende Bundle wird vorab ausschliesslich aus dem vollstaendig geprueften, nichtdeployenden Kandidatencommit unter `raw.githubusercontent.com/Torsten2025/washraum-app/<commit>/scripts/` in ein neues privates `/tmp`-Verzeichnis geladen. Der vom Validator uebergebene SHA256 des Manifestes wird zuerst geprueft; danach prueft `sha256sum -c` jedes Bundleartefakt, bevor Node mit `NODE_PATH=/opt/render/project/src/node_modules` startet. Kein `master`-Checkout, kein Deploy und kein frei kopiertes Skript gehoeren zu diesem Lieferweg. Der Bootstrap liest die tatsaechliche Liveversion aus der regulaeren unverlinkten Datei `/opt/render/project/src/package.json`. Quelle und Zielverzeichnis muessen regulaer und unverlinkt sein; ein vorhandenes Ziel, eine Parallel-Lockdatei, Symlink/Reparsepunkt oder jede Identitaetsabweichung stoppt ohne Backupwirkung. Die Quelle wird zuerst ausschliesslich read-only auf SQLite-Integritaet, Fremdschluessel und den allowlisteten WaschZeit-Schemavertrag geprueft; eine fremde oder defekte Datenbank erzeugt daher auch keine temporaere Datei. Danach erstellt SQLite online genau eine konsistente Kopie. Integritaet, Fremdschluessel, allowlistetes Schema, Schemahash und aggregierte Tabellenzaehler werden geprueft; eine isolierte temporaere Restorekopie muss dieselben Hashes und Zaehler besitzen. Nur der erfolgreiche Bootstrap erzeugt mit dem getrennten Owner-/CI-Schluessel einen signierten, frischen Einmalnachweis ueber Ziel, Laufzeitidentitaet, Backupdatei und Restore-PASS. Der Workflow akzeptiert keine frei eingegebenen Hashes oder selbst gesetzten PASS-Bools. Vor Livecheck und Hook verbraucht er den Proof-Nonce genau einmal durch das atomare Anlegen einer nicht deployenden GitHub-Tag-Ref, deren Name nur einen Hash aus Nonce und Kandidatencommit enthaelt. Existiert die Ref bereits, ist der Ledgerstatus unklar oder bricht der Lauf nach Verbrauch ab, endet der Vorgang ohne Retry und ohne weiteren Live-/Hookversuch. Danach vergleicht der Workflow den realen Health-Commit samt Liveversion und sendet hoechstens einen Hookrequest. Ausgaben enthalten keine Zeilen, Namen, IDs, Secrets oder PII. Ein Fehler entfernt nur eigene partielle beziehungsweise temporaere Dateien und loest weder Deploy noch Providerkontakt aus.

Die Produktionsquelle wird unter Linux einmal read-only geoeffnet; Strukturpruefung und SQLite-Onlinebackup verwenden dieselbe durchgehend offene SQLite-Verbindung und damit dasselbe bereits geoeffnete Dateiobjekt. Es gibt dazwischen kein zweites Oeffnen des veraenderbaren Pfades. Ein zusaetzlicher read-only Dateideskriptor bindet Geraete-/Inodeidentitaet; diese Identitaet und der unverlinkte Quellpfad werden an jedem relevanten Uebergang erneut verglichen. Ein Austausch des Pfadziels stoppt fail-closed und entfernt jedes eigene Teil- oder bereits publizierte Backup, bevor ein PASS-Nachweis entstehen kann.

Fuer den Nachfolgekandidaten `0.3.13` gilt die Versionsbindung an die bestehende Liveversion `0.3.11` und die Kandidatenversion `0.3.13`. Die separat dokumentierte Backup-Schemakorrektur ist Bestandteil dieses Kandidaten; die Versionsvorbereitung fuegt ihr keine weitere Funktion hinzu und erteilt keine Deployfreigabe.

Der Validator uebergibt nach dem finalen Commit sowohl `CANDIDATE_COMMIT` als auch den SHA256 der Manifestdatei. In der Render Runtime Shell ist ausschliesslich folgendes Rezept zulaessig; Platzhalter werden nicht geraten. Der Downloadpfad ist der exakte Commit, Redirects sind verboten, das private Ziel entsteht neu per `mktemp`, und erst zwei erfolgreiche Hashstufen erlauben den Start:

```bash
set -eu
umask 077
CANDIDATE_COMMIT='<validator-gepruefter-40-stelliger-commit>'
BUNDLE_MANIFEST_SHA256='<validator-gepruefter-64-stelliger-sha256>'
EXPECTED_LIVE_COMMIT="$RENDER_GIT_COMMIT"
case "$CANDIDATE_COMMIT$BUNDLE_MANIFEST_SHA256$EXPECTED_LIVE_COMMIT" in *[!0-9a-f]*|'') exit 1 ;; esac
[ "${#CANDIDATE_COMMIT}" -eq 40 ]
[ "${#BUNDLE_MANIFEST_SHA256}" -eq 64 ]
[ "${#EXPECTED_LIVE_COMMIT}" -eq 40 ]
BUNDLE_DIR="$(mktemp -d /tmp/waschzeit-backup-bundle.XXXXXX)"
trap 'case "$BUNDLE_DIR" in /tmp/waschzeit-backup-bundle.??????) rm -rf -- "$BUNDLE_DIR" ;; esac' EXIT
mkdir "$BUNDLE_DIR/scripts"
BASE="https://raw.githubusercontent.com/Torsten2025/washraum-app/$CANDIDATE_COMMIT/scripts"
for FILE in production-backup-bundle.sha256 bootstrap-production-backup.js production-backup-contract.js verify-production-backup.js; do
  curl --fail --silent --show-error --proto '=https' --tlsv1.2 --max-redirs 0 \
    "$BASE/$FILE" --output "$BUNDLE_DIR/scripts/$FILE"
done
printf '%s  %s\n' "$BUNDLE_MANIFEST_SHA256" "$BUNDLE_DIR/scripts/production-backup-bundle.sha256" | sha256sum -c -
(cd "$BUNDLE_DIR" && sha256sum -c scripts/production-backup-bundle.sha256)
NODE_PATH=/opt/render/project/src/node_modules node "$BUNDLE_DIR/scripts/bootstrap-production-backup.js" \
  --service washraum-app \
  --expected-live-commit "$EXPECTED_LIVE_COMMIT" \
...
Der GitHub-Workflow `.github/workflows/deploy-render.yml` startet nicht mehr automatisch bei einem Push. Er wird erst nach dem getrennten Produktions- und Wartungsgate manuell auf dem exakt getesteten Kandidatencommit gestartet. Statt frei eingebbarer Hashes und PASS-Bools akzeptiert er nur den hoechstens 30 Minuten alten, vom realen Produktionsbootstrap kryptografisch signierten Backup-/Restore-Nachweis; ein separates GitHub-Secret prueft dessen Unveraendertheit. Vor jedem externen Livecheck verbraucht der Workflow den signierten Nonce durch genau einen atomaren GitHub-Ref-Create. Diese Tag-Ref liegt ausserhalb des deployenden Branches und enthaelt nur einen SHA256 aus Nonce und Kandidatencommit. Bereits vorhandene Ref, Ledgerfehler oder ein unklarer Transportzustand stoppen terminal; auch ein nach Verbrauch abgestuerzter Lauf darf nicht wiederholt werden. Erst danach wird `/api/health` ohne Retry gegen den signierten Livecommit und die Liveversion geprueft. Der Workflow installiert Chromium, fuehrt `npm run check` und den verbindlichen Browserlauf aus und sendet ueber `scripts/trigger-production-deploy.js` hoechstens einen POST an den geheimen Render Deploy Hook. Redirect, Retry, Queue-Antwort, Timeout, Transportfehler oder unerwarteter Status beziehungsweise Antwortkoerper enden terminal ohne zweiten Request. Produktion gilt erst als aktuell, wenn `/api/health` den erwarteten Git-Commit meldet. Screenshots bleiben 14 Tage als CI-Artefakt erhalten; Deployhook und Proof-Schluessel bleiben getrennte Repository-Secrets.

Die Workflow-Auswahl `render_auto_deploy` muss als `type: choice` mit der einzigen explizit quotierten Option `"off"` definiert bleiben. Der uebergebene Wert ist weiterhin der String `off`; die Quotierung verhindert eine Boolean-Deutung unter YAML1.1, ohne die Auswahl oder den Schutz in `trigger-production-deploy.js` aufzuweichen. Ein lokaler Parser-PASS allein beweist keine erfolgreiche serverseitige Dispatchvalidierung. Nach einer Korrektur dieses Vertrags ist die Annahme durch GitHub separat mit dem freigegebenen Verfahren nachzuweisen; ein HTTP422 darf nicht durch einen blinden erneuten Produktionsdispatch beantwortet werden.

Der read-only Backup-Strukturvertrag erlaubt die von der App erzeugte Tabelle `calendar_feed_tokens`. Ist sie vorhanden, muessen die Spalten `id`, `user_id`, `apartment_id`, `house_id`, `token_hash`, `created_at` und `revoked_at` sowie die drei Fremdschluessel auf `users(id)`, `apartments(id)` und `houses(id)` mit `ON DELETE CASCADE` vorhanden sein. Aeltere Sicherungen ohne Kalenderfeed-Tabelle bleiben pruefbar. Unbekannte Tabellen, unvollstaendige Kalenderfeed-Strukturen und verwaiste Fremdschluessel bleiben gesperrt. Die lokale Regression startet den echten Server ohne Backup-, E-Mail- oder Pushkanal und prueft dessen Datenbank einschliesslich synthetischem Kalenderfeed-Eintrag, Online-Backup und isolierter Restorekopie; keine Tokeninhalte werden ausgegeben.

Die Tagesdetails verwenden fuer Normalbuchungen dieselbe datensparsame Wohnungsnamensaufloesung wie die Buchungsliste, einschliesslich vorhandener zusammengefuehrter Konten. Dauertermine verwenden ihre aktive Wohnungszuordnung im selben Haus. Fehlt eine gueltige Zuordnung, erscheint weiterhin neutral `Belegt`; Loginname, E-Mail-Adresse und technische Nutzer-/Wohnungskennungen werden nicht ausgegeben. Auch gespeicherte vergangene Buchungen bleiben mit lesbarem Namen sichtbar: `Vorbei` steht getrennt am Slot, ohne die gesamte Karte auszublenden. Lange Namen umbrechen innerhalb der Karte. Geloeschte historische Termine werden nicht rekonstruiert. Im Hausmodus `Liberal` zeigt die Tumblerzeile freie Kapazitaet ohne Reservehinweis; der Hinweis `einer bleibt frei` gilt ausschliesslich fuer `GBMZ-Regeln`.

Die Kalenderfeed-Regression prueft auf derselben geheimen Abo-Adresse neu gespeicherte, verschobene und geloeschte Normalbuchungen sowie deaktivierte Dauertermine. Eine Terminverschiebung behaelt ihre stabile UID; fremde Haeuser bleiben ausgeschlossen. Diese lokalen Serverpruefungen belegen keine bestimmte Abruffrequenz oder Live-Synchronisierung externer Kalender-Apps wie FamilyWall.

Abgrenzung der Versionsvorbereitung `0.3.12`: `render.agent-test.yaml` bleibt als separater Agent-Test-Blueprint unveraendert auf `agent-v0.3.10`. Seine bereits bestehende Abweichung vom Fixture-Versionsvertrag ist ein getrenntes Bestandsproblem und kein Bestandteil dieses Produktionspakets. Daraus folgt weder eine Agent-Test-Umgebungsanpassung noch eine Start- oder Deployfreigabe; vor einem kuenftigen Agent-Test-Release muss diese Bindung separat abgeglichen werden.

## Aenderungsprotokoll

- 06.09.2026: Produktionslinien-Kandidat `0.3.13` nach dem lokal unabhaengig geprueften Workflow-Choicefix vorbereitet. Aktive Paket-/Lock-, PWA-/News-, Test-/Fixture- und Backup-Kandidatenbindungen sind konsistent nachgefuehrt; die erwartete Liveversion bleibt `0.3.11`. Das Backup-Bundlemanifest folgt dem versionsgeaenderten Vertrag. Dependencies, Umgebungen, Blueprints und historische Belege bleiben unveraendert; lokale/Test-Kontexte bleiben `WaschZeit Test`. Keine weitere Funktionsaenderung, keine behauptete GitHub-Serverannahme und kein Commit-/Push-/Dispatch-/Deployauftrag durch diese Versionsvorbereitung.

- 06.09.2026: Die Produktionsworkflow-Auswahl `render_auto_deploy` verwendet explizit den quotierten String `"off"`. Ein genehmigter nichtdeployender Diagnoselauf hatte die serverseitige Ablehnung des unquotiert definierten Auswahlwerts mit HTTP422 bestaetigt; der konkrete interne GitHub-Parser bleibt unbekannt. Der String ist damit unter YAML1.1 und YAML1.2 eindeutig, eine Release-Regression sichert die Schreibweise. Inputtyp, Commit-/Proof-/AutoDeploy-Pruefungen und Einmaldeploy-Schutz bleiben unveraendert. Lokale Korrektur ohne Versionswechsel oder Auslieferungsfreigabe; ein echter serverseitiger Nachtest bleibt separat erforderlich.

- 05.09.2026: Kandidat `0.3.12` als reine Versionsfortschreibung des lokal unabhaengig abgenommenen Funktionsstands vorbereitet. Paket/Lock, PWA-Cache, Aktualisierungshinweis, Fixture-Versionsvertrag und Releasepruefungen sind konsistent; der Backup-Releasevertrag bindet Liveversion `0.3.11` an Kandidat `0.3.12`, sein Bundle-Manifest wurde neu berechnet. Keine weitere Funktionsaenderung, kein Commit, Push, Deployment oder Produktionszugriff durch diesen Vorbereitungsschritt; Auslieferung bleibt separat freizugeben.

- Tagesdialog-Namensanzeige und Lesbarkeit korrigiert: verwaltete Namen fuer belegte Normal-/Dauertermine, eigene Buchungen, bestehende Altkonten und vergangene gespeicherte Termine bleiben im autorisierten Haus klar erkennbar. Browserregression fuer DE/EN und Breiten 390, 760 und 1024 prueft lange Namen, Umbruch und unverminderten Kontrast. Tumblerreserve-Hinweise sind auf GBMZ begrenzt. Kalenderfeed-Lebenszyklus und unveraenderte UID sind lokal regressionsgetestet; kein Feedprotokoll und keine externe Kalender-App wurden umgebaut.
- Nach erfolgreicher Stoerungsuebernahme bleibt der Fall durch den Wechsel von `Neu` zu `In Bearbeitung` sichtbar. Fehler lassen den bisherigen Filter bestehen; die Regression prueft beide Wege.

- Backup-Befund `QA-BACKUP-SCHEMA-20260905` korrigiert: Die bislang fehlende Kalenderfeed-Tabelle ist mit ausdruecklichen Mindestspalten und Fremdschluesselstruktur im Backup-Vertrag erlaubt. Ein echter App-Schemaaufbau mit synthetischem Feed-Eintrag sowie negative Fremdtabellen-, Spalten-, Verknuepfungs- und Integritaetsfaelle sichern die Korrektur ab. Das Bootstrap-Bundle-Manifest folgt dem geaenderten Vertrag; Produktionszugriffe und Deployfreigaben sind damit nicht erteilt.

- Produktionskandidat `0.3.11` loest drei sichtbare Pilotbefunde ohne neue Rollen- oder Datenrechte: Belegte Termine ermitteln den adminverwalteten Klingelschild-/Wohnungsnamen auch fuer bestehende, zusammengefuehrte Konten und verwenden nur dann neutral `Belegt`, wenn im aktiven Haus wirklich keine fachliche Wohnungsbezeichnung existiert. Loginname, E-Mail und technische IDs bleiben ausgeschlossen. Die Kopfzeile wechselt nun bereits bei schmalen Desktop-PWA-Fenstern in das responsive Raster, sodass Aktionen nicht rechts abgeschnitten werden und kein horizontaler Seitenlauf notwendig ist. Der Kalenderfeed erklaert Erstellen, Kopieren und Abonnieren in drei Schritten und kann die nur einmal sichtbare Geheimadresse direkt per `webcal:` an eine Kalender-App uebergeben; beim Schliessen wird auch das Linkziel entfernt. Nach dem Update erscheint pro Browser/Geraet einmalig der kompakte Hinweis `Neu in Version 0.3.11`; `Verstanden` merkt die Version lokal und blendet ihn danach nicht erneut automatisch ein. Der Hinweis erklaert auch die hausbezogene Buchungslogik: Der Superadmin kann bei der Hausanlage und spaeter zwischen `GBMZ` (bisherige Regeln) und `Liberal` (freie Eintragung in verfuegbare Slots, weiterhin ohne Doppelbelegung und nur im berechtigten Haus) wechseln. Der Backup-Ablauf bleibt funktional unveraendert; lediglich seine erwartete Live-/Kandidatenversion folgt der Releasekette `0.3.10 -> 0.3.11`.
- Produktionskandidat `0.3.10` vereinfacht den Stoerungsablauf auf `Reparatur -> Funktionspruefung`: Eine erfolgreiche Pruefung schliesst den Fall und gibt eine fallbezogen gesperrte Ressource atomar frei; ein Fehlschlag bleibt offen und gegebenenfalls gesperrt. Bestehende Chronik und historische `tested`-Faelle bleiben erhalten und abschliessbar. Hausadmin-Intro, Transkripte und Untertitel beschreiben denselben Ablauf ohne zusaetzliche Abschlussnotiz.
- Produktionskandidat `0.3.10` bindet den Produktionsdeploy an ein frisches, laufzeitgebundenes SQLite-Online-Backup mit Integritaets-, Fremdschluessel-, Schema-, Zaehler- und isoliertem Restore-Nachweis. Der Nachweis ist kryptografisch signiert; erfundene Eingaben oder fehlender realer Live-Health-Abgleich stoppen vor dem Hook. Ein atomarer GitHub-Ref-Ledger verbraucht jeden Proof vor Livecheck genau einmal; Replay, parallele Wiederverwendung und unsichere Crash-/Transportgrenzen stoppen ohne zweiten Versuch. Das vor dem Deploy benoetigte Bootstrap-Bundle wird ausschliesslich aus dem exakten Kandidatencommit geladen und vor Ausfuehrung gegen sein gefrorenes SHA256-Manifest geprueft. Der GitHub-Workflow ist nur noch manuell startbar und sendet nach allen Gates hoechstens einen Render-Hookrequest ohne Retry oder Redirect.
- Produktionskandidat `0.3.10` zeigt in belegten Kalenderslots datensparsam den verwalteten Klingelschild-/Wohnungsnamen der Buchungspartei, jedoch nur im serverseitig autorisierten aktiven Haus. Die eigene gemeinsame Wohnungspartei erscheint lokalisiert als `Du`/`You`; ohne aktive Wohnungszuordnung bleibt neutral `Belegt`/`Occupied`. Loginname, E-Mail, Kontakt- und technische Identitaetsfelder werden nicht an die Kalenderansicht ausgegeben. Kalenderfeed, Audit, Export, Benachrichtigungen und Speicherung bleiben unveraendert.
