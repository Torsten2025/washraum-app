...
Ein neues Dauerpaket enthaelt genau eine Waschmaschine sowie optional hoechstens einen Trockenraum und einen Tumbler. Seine Bestandteile besitzen eine gemeinsame Gruppenkennung; auch ein Paket nur mit Waschmaschine erhaelt diese Kennung. Beim Trockenraum wird die zusammenhaengende Nutzungsdauer als Anzahl der belegten, fuer den Waschslot erlaubten Zeitfenster gespeichert. Ein nicht zulaessiges Sonntagsfenster wird abgewiesen. Hauszuordnung, aktive Ressourcen, doppelte Angaben, bestehende normale und feste Buchungen sowie die Tumblerreserve werden vor und innerhalb der Transaktion erneut geprueft. Ein Konflikt speichert keinen Bestandteil. Beim Entfernen wird das vollstaendige aktive Paket desselben Hauses deaktiviert. Historische Legacy-Einzeltermine mit `group_id = NULL` bleiben separat les- und entfernbar. Auch bei festen Tumbler-Terminen bleibt mindestens ein Tumbler frei.
...
| `npm run test:production-backup` | Produktionsguard, additive Migration, Off-Disk-Backup/Readback, Restore, Scheduler, Frische-Schreibsperre, Provider-Hold und Rollbackvertrag mit lokalen Fakes pruefen |
| `npm run production:predeploy` | Separates produktives Predeploy-Gate: read-only DB-Oeffnung und frisches verifiziertes verschluesseltes Off-Disk-Backup; nur nach eigenem Produktionsauftrag ausfuehren |
...
`render.yaml` beschreibt ausschliesslich einen spaeteren, getrennt freizugebenden Produktionsdeploy auf `master`, installiert mit `npm ci` und behaelt die vorhandene SQLite-Datenbank auf der persistenten Disk unter `/var/data`. Vor irgendeiner Datenbank-, WAL-, Schema-, Seed- oder Settingwirkung muessen Produktionsdienst, URL, voller Commit, Version, Release, persistenter DB-Pfad, Einzelinstanz, Schema, Provider-Hold, Backupvertrag, Kopia-Pin und getrennte Runtimegeheimnisse exakt stimmen. Noch vor `mkdir`, `mkdtemp`, `chmod`, Konfigurations-/Cacheanlage oder Subprozessstart wird das Kopia-Artefakt ueber seinen kanonischen direkten Pfad als regulaere lesbare Datei geoeffnet und gegen den vollstaendigen Release-Pin sowie SHA256 geprueft; Dateityp-, Lesbarkeits-, Pfad-/Handle- oder Hashdrift stoppt mit null Schreibwirkung. Vor jedem Kopia-Aufruf werden zuerst Cache-, Config-, Temp- und Argumentplan vollstaendig vorbereitet. Erst danach wird dieselbe Binaeridentitaet unter Linux mit `O_RDONLY|O_NOFOLLOW|O_CLOEXEC` erneut geoeffnet und auf genau diesem offenen Handle gegen Dateityp, Groesse, unveraenderte Device-/Inode-/Zeitidentitaet, Schreib-/Set-ID-Rechte, ELF64/x86-64, den erlaubten `PT_INTERP`-Vertrag und SHA256 geprueft. Der validierte Parent-FD wird beim synchronen `spawn` explizit auf Child-FD 3 vererbt und ausschliesslich ueber `/proc/self/fd/3` ausgefuehrt; der veraenderbare Originalpfad ist kein Ausfuehrungsfallback. Der Parent-FD bleibt bis zum abgeschlossenen synchronen Startversuch offen und wird bei Erfolg wie Fehler genau einmal geschlossen. Nicht-Linux, fehlendes `/proc/self/fd`, Mapping-, Identitaets- oder Startversagen stoppt. Lokale Windows-Tests belegen dabei nur den injizierten Adapter- und Fehlervertrag; der echte Linux-/Render-/Kopia-Lauf bleibt ein externes Produktionsgate. Ein Ersatz waehrend der Vorbereitung oder zwischen zwei Kommandos stoppt vor dem naechsten Subprozess. Die Datenbank wird zuerst `readonly` und `fileMustExist` geoeffnet; unbekannter Schema- oder Integritaetsdrift stoppt.

Produktionsmigrationen liegen ausschliesslich als releasegebundene JSON-Artefakte unter `migrations/production/`. Das konkrete Artefakt entsteht erst nach der read-only Live-Schemainventur und bindet Appversion, vollen Commit, kanonischen Artefakt-SHA256, Vor-/Nach-Schemahash, Codekompatibilitaet und `destructive=false`. Erlaubt sind nur additive Tabellen, Indizes, Trigger und Spalten. Ledger und Schritte laufen gemeinsam in einer Transaktion; Fachzaehler und Fremdschluessel muessen unveraendert bleiben. Ein zweiter Lauf ist No-op. `DROP`, Backfill, Normalisierung, Scrub, Cleanup und implizite Datenloeschung sind gesperrt.

Vor einem Deploy wird `npm run production:predeploy` als eigenes Gate gegen die noch laufende Revision ausgefuehrt. Vor der ersten schreibenden DB-Oeffnung beim neuen Start entsteht nochmals zwingend ein konsistenter SQLite-Online-Snapshot. Beide Sicherungen werden clientseitig verschluesselt off-disk gespeichert, remote vollstaendig verifiziert, in einen neuen isolierten Pfad zurueckgelesen und dort gegen Manifest, SHA256, Schema, `integrity_check` und `foreign_key_check` geprueft. Nach der Migration folgt dieselbe Pruefung erneut. Fehlt einer dieser Belege, startet kein Listener; vor dem Premigrationsbackup findet kein DB-Write statt.

Im laufenden Dienst erzeugt ein Single-Flight-Scheduler mit 240-Sekunden-Intervall und begrenzten Kommando-/Jobbudgets periodische verifizierte Backups. Ueberlappende Ticks werden verworfen, ohne einen zweiten Lauf zu starten. Ist das letzte vollstaendig verifizierte Remote-Readback 300 Sekunden alt oder aelter, antworten neue fachliche `POST`-/`PUT`-/`PATCH`-/`DELETE`-Aktionen mit `503`, bis ein Backup wieder gruen ist. Login, Logout und Keepalive bleiben als reine Sitzungsaktionen ausgenommen. Normale Buchungsantworten warten nicht auf einen eigenen Snapshot; es gibt keine neue Request-Idempotenz-, Receipt-, Remote-Ack-, Queue- oder Backpressure-Schicht. Das zugesagte RPO von hoechstens fuenf Minuten gilt ausschliesslich fuer vollstaendigen Diskverlust und nur solange dieses Frischegate und die externen R2-/Kopia-Gates tatsaechlich PASS sind. RTO-Ziel ist hoechstens vier Stunden.

Kopia `0.23.1` ist ausschliesslich ueber das gepinnte offizielle Linux-x64-Artefakt gebunden und akzeptiert nur einen geschuetzten lokalen Snapshotpfad, keine Rohbytes. Der App-Service kennt niemals R2-/AWS-/Bucket-Credentials oder eine direkte Repositorykonfiguration. Er verbindet sich ausschliesslich per kanonischer HTTPS-URL und gepinntem TLS-Zertifikatsfingerabdruck mit einem getrennten Kopia Repository Server. Sein dediziertes Konto ist fest auf `APPEND_READ` begrenzt; Full-, Maintenance- und Delete-Rechte sind im App-Prozess verboten. Der Kindprozess erhaelt nur das Server-Kontogeheimnis und eine minimale System-Allowlist, keine Providerwerte. Die Verifikation eines formellen isolierten Restore-Drills verwendet einen getrennten `READ_ONLY`-Adapter; Owner-/Recovery-/Full-Credentials werden weder im App-Service noch in dessen Blueprint provisioniert. Appseitige Retentionloeschung existiert nicht. Die vorlaeufigen 30 Tage werden spaeter durch einen getrennten externen R2-Policyvertrag durchgesetzt. Repository Server, R2-EU-Bucket, Server-zu-R2-Credentials, ACL-Einrichtung, Checkout/Kosten, Recovery-Key-Verantwortliche, Legal/Datenschutz, Retention/Lock und ein echter Restore-Drill bleiben externe Produktionsgates. Dieser Kandidat enthaelt weder Kopia-Binaerdatei noch R2-Werte und hat keine echte Sicherung, Loeschung oder Provideraktion ausgefuehrt.

E-Mail, Push, Reminder und deren Providerwrapper bleiben beim ersten Produktionsstart durch `PRODUCTION_PROVIDER_HOLD=true`, deaktivierte Schalter und fehlende SMTP-/VAPID-Werte fail-closed. Testfixture, Testkonten, Testlinks, Legacy-Registrierung und Agent-Test-Fake-Sink sind in Produktion vor Wirkung gesperrt. Code- und Datenrollback werden getrennt geplant; ein Datenrestore verlangt isolierten Drill, Schreibstopp und ein ausdrueckliches Owner-Gate. Der Staging-Blueprint darf niemals Produktionsdisk, Produktions-Env-Gruppe, R2-Repository oder reale Domain uebernehmen.
...
- Vorabversion `0.3.0-test.14` auf sauberer `test.12`-Basis fuer den Produktions-Datenerhalt vorbereitet. Der verworfene synchrone Request-/Remote-Ack-/Receipt-/Idempotenz-/Queuepfad ist nicht enthalten. Neu sind der wirkungsfreie Produktionsguard, read-only Schemaidentitaet, additives unveraenderbares Migrationsledger, verifiziertes Predeploy-/Premigrations-/Postmigrationsbackup, isolierter Restore, getrennter Code-/Datenrollback, Provider-Hold und ein periodischer Single-Flight-Backupvertrag mit globaler Schreibsperre ab Erreichen von fuenf Minuten ohne verifiziertes Remote-Readback. Direkte R2-/AWS-/Bucket-Credentials sind im App-Service verboten; Kopia nutzt nur einen TLS-gepinnten Repository Server mit `APPEND_READ`-Appkonto und einen getrennten `READ_ONLY`-Verifier fuer den isolierten Drill. Binary-Pfad, regulaerer Dateityp, Lesbarkeit, kompletter Release-Pin und SHA256 werden vor jeder FS-Schreibwirkung und nach saemtlicher Cache-/Config-/Temp-/Argumentvorbereitung synchron auf demselben offenen Handle geprueft; der Subprozess erbt exakt diesen Handle als `/proc/self/fd/3`, ohne Pfadfallback. Lokale Windows-Pruefungen bleiben Adapter-/Fakebelege, waehrend echter Linux-/Render-/Kopia-Vollzug ein externes Produktionsgate ist. Kopia/R2 bleiben gepinnte Adapter/Fakes ohne echten Vollzug; der bestehende `test.12`-Produkt-, Rollen-, Haus- und Restplatzstand blieb unveraendert.
