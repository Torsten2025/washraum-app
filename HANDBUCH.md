...
Die Produktion besitzt eine zusaetzliche harte Hold-Grenze: `BACKUP_ENABLED`, `AUTO_BACKUP` und `EMAIL_ENABLED` muessen jeweils explizit `false` sein. Push darf nur mit `PUSH_ENABLED=true` und gleichzeitig `PRODUCTION_PUSH_APPROVED=true` starten; ohne diese doppelte Freigabe bleibt Push deaktiviert oder der Start stoppt vor Wirkung. Test-Fixture, Testlink, Legacy-Registrierung und Seed-Passwortreset muessen fehlen oder exakt `false` sein. Externe Backup-Upload-, SMTP-, VAPID-, R2-, S3-, AWS-, Cloudflare- oder Kopia-Bindungen muessen vollstaendig fehlen. Bei einer Abweichung endet der Start vor Verzeichnisanlage, SQLite, WAL, Schema, Seed, Cleanup, Scheduler oder Providerkontakt.
